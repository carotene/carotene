...
-record(state, {channel, supervisor, exchange_supervisor, queue_supervisor}).
...
init([Sup]) ->
    ...
    self() ! {start_redis_queue_sup, Channel},
...
handle_info({start_redis_queue_sup, Channel}, State = #state{supervisor = Sup}) ->
    {ok, QueueSup} = supervisor:start_child(Sup, {redis_queue_sup,
          {redis_queue_sup, start_link, [Channel]},
          permanent,
          infinity,
          worker,
          [redis_queue_sup]}),
